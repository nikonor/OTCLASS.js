...
Тут стоит сделать важное замечание. Допустим, что наше поле, которое мы меняем, будет вложено глубоко внутрь. Например,
...
И нам надо заменить bdate при id=2, тогда команда будет выглядеть так:
  
    ot.update_item({'field':'id','eq':2},'dates.bdate','1.1.2011');

Т.е. указываем поля через **точку**.

_И ни в коем случае тут нельзя использовать вызов update, потому как тогда в dates останется только bdate_
...
Во всех примерах, что приведены выше, условие выглядело как 
  
  {'field':'id','eq':22}

Но это упрощённый вариант. На самом деле этот параметр может быть массивом таких словарей. Пока, правда, реализовано только условие точное сравнение eq, чуть позже будут реализованы условия остальных типов
